...
function Item(props) {
  ...
  function opencolor() {
    if (state.openpalette === false)
      setState((prev) => {
        return { ...prev, openpalette: true };
      });
  }

  function closecolor() {
    setState((prev) => {
      return { ...prev, openpalette: false };
    });
  }
  ...
  function clickcolor(event) {
    ...
    setState((prev) => {
      return {
        ...prev,
        mycolor: x,
        itemcode: [...state.itemcode, { id: props.id, color: state.mycolor }],
      };
    });
  }
    ...
    <>
      <div className="itm" style={state.mycolor} onMouseLeave={closecolor}>
        <div>
          ...
          <p id="p2">{props.passarr.content}</p>
        </div>

        <div className="icons">
          <button onClick={opencolor} title="background color">
            <PaletteIcon />
          </button>
          ...
          <button
            onClick={() => {
              setState((prev) => {
                return { ...prev, mycolor: { backgroundColor: "white" } };
              });
        ...
        <div
          className="color"
          style={{ display: state.openpalette ? "block" : "none" }}
        >
